        ...
        if (sequenceData) {
            sequence = (
                <div style={{margin: "10px"}}>
                    <h1 style={{display: "inline"}}>{this.props.sequenceSelector + ": "}</h1>
                    <table style={{display: "inline-block"}} className={classes.MatrixTable}>
                        <tbody>
                            <tr key={"only-row"}>
                                ...
                                <td key="oeis" className={classes.MatrixCell}>
                                    <a href={"http://oeis.org/search?q=" + sequenceData.join("%2C") + "&language=english&go=Search"} target="_blank">
